...
void wiced_sense_create(void)
{
	...
	gpio_configurePin((GPIO_PIN_P28) / 16, (GPIO_PIN_P28) % 16, GPIO_INPUT_DISABLE, 0);
...
void wiced_sense_connection_up(void)
{
	...
	// notifications needed 5ms before the TX opportunity (so we have enough time to poll sensors
	//         and enqueue the notification)  = 5000uS / 625uS in BT slots
...
void wiced_sense_advertisement_stopped(void)
{
...

	{
		...
		gpio_configurePin(GPIO_PIN_P0 / 16, GPIO_PIN_P0 % 16, GPIO_INPUT_ENABLE | GPIO_PULL_DOWN, 0);
...
int wiced_sense_write_handler(LEGATTDB_ENTRY_HDR *p)
{
    ...
    // do some noise
    wiced_sense_short_beep_buzzer(WICED_SENSE_BUZZER_MID_HIGH);
